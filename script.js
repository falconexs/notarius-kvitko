 ...
 *  8. AccessibilityMode    – version for visually impaired visitors
 */
...
(function MobileMenu() {
  ...
  // Close button inside mobile menu (explicit control)
  const mobileCloseBtn = document.getElementById('mobileClose');
  if (mobileCloseBtn) {
    // Use a capture-phase listener and stopImmediatePropagation to ensure
    // the close action runs before any other handlers and reliably closes
    // the menu even if other scripts attach interfering listeners.
    mobileCloseBtn.addEventListener('click', function (e) {
      e.preventDefault();
      e.stopImmediatePropagation();
      // Mirror the logic of close() explicitly
      isOpen = false;
      try {
        burger.classList.remove('open');
        burger.setAttribute('aria-expanded', 'false');
...
/* ================================================================
   ...
   Toggles a high-contrast, larger-text version of the site.
   ================================================================ */
(function AccessibilityMode() {
  const toggle = document.getElementById('accessibilityToggle');
  if (!toggle) return;

  const STORAGE_KEY = 'notary-accessibility-mode';
  ...
  const saved = window.localStorage.getItem(STORAGE_KEY) === 'on';
  applyState(saved);
...
/* ================================================================
   6. INTERSECTION REVEAL ANIMATIONS
...
/* ================================================================
   9. PHONE LINK FORMATTING (UX enhancement)
